...
function single_page_shortcode_function() {
    ...
    if (empty($gallery)) {
        $gallery = get_post_meta(get_the_ID(), 'media_property_photos', true);
    }

    if (!is_array($gallery)) {
        $maybe_unserialized = maybe_unserialize($gallery);
        if (is_array($maybe_unserialized)) {
            $gallery = $maybe_unserialized;
    ...
    if (is_array($gallery)) {
        $gallery = implode(',', array_filter(array_map('absint', $gallery)));
    } elseif (is_string($gallery)) {
        $gallery = preg_replace('/\s+/', '', $gallery);
    }
    ...
    $saved_amenities = get_post_meta(get_the_ID(), 'cba_property_amenities', true);
    ...
    $amenity_items = [];
    foreach ($saved_amenities as $amenity_key) {
        if (isset($amenity_labels[$amenity_key])) {
            $amenity_items[] = $amenity_labels[$amenity_key];
        }
    }
    $amenity_items = array_values(array_unique($amenity_items));
    $amenity_items = array_slice($amenity_items, 0, 4);
