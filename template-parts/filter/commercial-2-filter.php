                ...
                <form class="index-filter idex_form_padding" id="commercialFilterForm">
                    ...
                    <div class="index-row index-row--top">
                        ...
                        <div class="commercial-search-wrap">
                            <input type="text" name="s" class="index-input" id="commercialLiveSearch" placeholder="Search by title or area..." autocomplete="off" />
...
function commercial_search_suggestions() {
    ...
    if (count($items) < 8) {
        ...
        $sql = $wpdb->prepare("
            ...
            LIMIT 8
        ", COMMERCIAL_INDEX_META_EXPIRED_DATE, COMMERCIAL_INDEX_CPT, COMMERCIAL_INDEX_META_AREA, $like, date('Y-m-d'));
        ...
        (function($){
            ...
            function renderCommercialSuggestions(items){
                ...
                items.forEach(function(item){
                    html += '<div class=\"commercial-search-item\" data-title=\"' + escapeHtml(item.title) + '\" data-url=\"' + escapeHtml(item.url) + '\">';
            ...
            function fetchCommercialSuggestions(term){
                $.post(ajaxUrl, {
                    action: 'commercial_search_suggestions',
                    nonce: $('#commercialFilterForm input[name=\"nonce\"]').val(),
                    term: term
                }).done(function(res){
            ...
            $(document).on('click', '.commercial-search-item', function(){
                ...
                $('#commercialLiveSearch').val(title);
                hideCommercialSuggestions();
                fetchProps(1);
            });
