...
if (!defined('COMMERCIAL_INDEX_META_CATEGORY')) {
    define('COMMERCIAL_INDEX_META_CATEGORY', '_property_category_prod');
}

if (!defined('COMMERCIAL_INDEX_META_PROPERTY_TYPE')) {
    define('COMMERCIAL_INDEX_META_PROPERTY_TYPE', '_property_type_com');
}

if (!defined('COMMERCIAL_INDEX_META_PROPERTY_TYPE_LEGACY')) {
    define('COMMERCIAL_INDEX_META_PROPERTY_TYPE_LEGACY', 'pp_property_type');
}
...
add_shortcode('porpertypi_ajax_filter_dynamic_commercial', function () {
    ...
    $purpose_options = commercial_get_distinct_meta_values([COMMERCIAL_INDEX_META_PURPOSE, 'pp_purpose']);
    $status_options  = commercial_get_distinct_meta_values([COMMERCIAL_INDEX_META_STATUS, 'pp_status']);
...
function commercial_filter_porpertypi_dynamic() {
    ...
    $meta_query[] = [
        'relation' => 'OR',
        [
            'key'     => COMMERCIAL_INDEX_META_CATEGORY,
            'value'   => 'Commercial',
            'compare' => '=',
        ],
        [
        ...
        [
            'key'     => COMMERCIAL_INDEX_META_PROPERTY_TYPE_LEGACY,
            'value'   => 'Commercial',
            'compare' => '=',
        ],
    ];
    ...
    if ($s !== '') {
        ...
        $search_meta_keys = [
            ...
            COMMERCIAL_INDEX_META_PROPERTY_TYPE_LEGACY,
            COMMERCIAL_INDEX_META_ADDRESS,
            COMMERCIAL_INDEX_META_CATEGORY,
        ];
    ...
    if ($q->have_posts()) {
        ...
        while ($q->have_posts()) {
            ...
            $price = (int) commercial_get_post_meta_value($id, [COMMERCIAL_INDEX_META_PRICE, '_re_price']);
            $beds  = (int) commercial_get_post_meta_value($id, [COMMERCIAL_INDEX_META_BEDS, 'spec_rooms', '_re_beds']);
            $baths = (int) commercial_get_post_meta_value($id, [COMMERCIAL_INDEX_META_BATHS, 'spec_bathrooms', '_re_baths']);
            $size  = (int) commercial_get_post_meta_value($id, [COMMERCIAL_INDEX_META_SIZE, 'spec_size_sqft', '_re_size_sqft']);
            $location = commercial_get_post_meta_value($id, [COMMERCIAL_INDEX_META_ADDRESS, 'pp_address']);

            $pval = commercial_get_post_meta_value($id, [COMMERCIAL_INDEX_META_PURPOSE, 'pp_purpose']);
            $sval = commercial_get_post_meta_value($id, [COMMERCIAL_INDEX_META_STATUS, 'pp_status']);
            ?>
 ...
 */
function commercial_get_post_meta_value($post_id, $meta_keys, $default = '') {
...
function commercial_get_distinct_meta_values($meta_keys) {
    global $wpdb;
    ...
    $query_args = array_merge($meta_keys, [COMMERCIAL_INDEX_CPT]);
