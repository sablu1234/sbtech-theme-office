...
define('RE_CPT_NEW_PROJECTS', 'porpertypi');

define('RE_META_PURPOSE_NEW_PROJECTS', 'pp_purpose'); // ✅ your real meta_key
define('RE_META_STATUS_NEW_PROJECTS',  'pp_status');  // ✅ your real meta_key

define('RE_META_PRICE_NEW_PROJECTS',   '_re_price');
define('RE_META_BEDS_NEW_PROJECTS',    '_re_beds');
define('RE_META_BATHS_NEW_PROJECTS',   '_re_baths');
define('RE_META_SIZE_NEW_PROJECTS',    '_re_size_sqft');

/**
 * Shortcode: [porpertypi_ajax_filter_dynamic_new_projects]
 */
add_shortcode('porpertypi_ajax_filter_dynamic_new_projects', function () {
    $nonce = wp_create_nonce('re_filter_nonce');

    // ✅ dynamic values from CPT meta
    $purpose_options = re_get_distinct_meta_values_new_projects(RE_META_PURPOSE_NEW_PROJECTS);
    $status_options  = re_get_distinct_meta_values_new_projects(RE_META_STATUS_NEW_PROJECTS);
    ...
    <div class="re-wrap">
        <div class="re-hero">
            ...
            <div class="re-hero__inner">
                <h2 class="re-hero__title">Find New Property</h2>
                    ...
                    <div class="re-row re-row--top">
                        <select name="purpose" class="re-input">
                            ...
                            <?php foreach ($purpose_options as $v): ?>
                                <option value="<?php echo esc_attr($v); ?>"><?php echo esc_html(re_pretty_label_new_projects($v)); ?></option>
                            <?php endforeach; ?>
                        </select>
 ...
 * AJAX
 */
add_action('wp_ajax_re_filter_porpertypi_dynamic_new_projects', 're_filter_porpertypi_dynamic_new_projects');
add_action('wp_ajax_nopriv_re_filter_porpertypi_dynamic_new_projects', 're_filter_porpertypi_dynamic_new_projects');

function re_filter_porpertypi_dynamic_new_projects() {
    ...
    if ($purpose !== '') $meta_query[] = ['key' => RE_META_PURPOSE_NEW_PROJECTS, 'value' => $purpose, 'compare' => '='];
    if ($status  !== '') $meta_query[] = ['key' => RE_META_STATUS_NEW_PROJECTS,  'value' => $status,  'compare' => '='];

    if ($min_price !== null) $meta_query[] = ['key' => RE_META_PRICE_NEW_PROJECTS, 'value' => $min_price, 'type' => 'NUMERIC', 'compare' => '>='];
    if ($max_price !== null) $meta_query[] = ['key' => RE_META_PRICE_NEW_PROJECTS, 'value' => $max_price, 'type' => 'NUMERIC', 'compare' => '<='];

    if ($min_beds  !== null) $meta_query[] = ['key' => RE_META_BEDS_NEW_PROJECTS,  'value' => $min_beds,  'type' => 'NUMERIC', 'compare' => '>='];
    if ($min_baths !== null) $meta_query[] = ['key' => RE_META_BATHS_NEW_PROJECTS, 'value' => $min_baths, 'type' => 'NUMERIC', 'compare' => '>='];

    if ($min_size !== null) $meta_query[] = ['key' => RE_META_SIZE_NEW_PROJECTS, 'value' => $min_size, 'type' => 'NUMERIC', 'compare' => '>='];
    if ($max_size !== null) $meta_query[] = ['key' => RE_META_SIZE_NEW_PROJECTS, 'value' => $max_size, 'type' => 'NUMERIC', 'compare' => '<='];
    ...
    if ($sort === 'price_asc') {
        ...
        $order = 'ASC';
        $meta_key = RE_META_PRICE_NEW_PROJECTS;
    }
    if ($sort === 'price_desc') {
        $orderby = 'meta_value_num';
        $order = 'DESC';
    ...
    if ($q->have_posts()) {
        ...
        while ($q->have_posts()) {
            ...
            $price = (int)get_post_meta($id, RE_META_PRICE_NEW_PROJECTS, true);
            $beds  = (int)get_post_meta($id, RE_META_BEDS_NEW_PROJECTS, true);
            $baths = (int)get_post_meta($id, RE_META_BATHS_NEW_PROJECTS, true);
            $size  = (int)get_post_meta($id, RE_META_SIZE_NEW_PROJECTS, true);

            $pval = get_post_meta($id, RE_META_PURPOSE_NEW_PROJECTS, true);
            $sval = get_post_meta($id, RE_META_STATUS_NEW_PROJECTS, true);
    ?>
            <a class="re-card" href="<?php echo esc_url(get_permalink($id)); ?>">
                <div class="re-card__img">
                    ...
                    <div class="re-badges">
                        <?php if ($pval !== ''): ?><span class="re-badge"><?php echo esc_html(re_pretty_label_new_projects($pval)); ?></span><?php endif; ?>
                        <?php if ($sval !== ''): ?><span class="re-badge re-badge--dark"><?php echo esc_html(re_pretty_label_new_projects($sval)); ?></span><?php endif; ?>
                    </div>
                </div>
 ...
 */
function re_get_distinct_meta_values_new_projects($meta_key) {
    global $wpdb;
...
function re_pretty_label_new_projects($v) {
    $v = trim((string)$v);
    $v = str_replace(['-', '_'], ' ', $v);
    $v = preg_replace('/\s+/', ' ', $v);
    return ucwords($v);
...
add_action('wp_enqueue_scripts', function () {
    ...
    .re-hero__bg{position:absolute;inset:0;background:linear-gradient(180deg,rgba(2,8,23,.35),rgba(2,8,23,.12)),url('https://images.unsplash.com/photo-1560518883-ce09059eeffa?q=80&w=1073&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D');background-size:cover;background-position:center}
.re-hero__inner {
    position: relative;
    padding: 24px;
    display: flex;
    ...
    form#reFilterForm {display: flex;flex-direction: column;gap: 9px;width: 90%;}
  ...
  ");
    ...
    (function($){
      ...
      function fetchProps(page){
        ...
        data.action = 're_filter_porpertypi_dynamic_new_projects';
        data.paged = page || 1;
