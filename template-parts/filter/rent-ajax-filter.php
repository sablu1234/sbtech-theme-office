                ...
                <form class="index-filter idex_form_padding" id="rentFilterForm">
                    ...
                    <div class="index-row index-row--top">
                        ...
                        <div class="rent-search-wrap">
                            <input type="text" name="s" class="index-input" id="rentLiveSearch" placeholder="Search by title or area..." autocomplete="off" />
...
function rent_search_suggestions() {
    ...
    if (count($items) < 8) {
        ...
        $sql = $wpdb->prepare("
            ...
            LIMIT 8
        ", RENT_INDEX_META_EXPIRED_DATE, RENT_INDEX_CPT, RENT_INDEX_META_AREA, $like, date('Y-m-d'));
        ...
        (function($){
            ...
            function renderRentSuggestions(items){
                ...
                items.forEach(function(item){
                    html += '<div class=\"rent-search-item\" data-title=\"' + escapeHtml(item.title) + '\" data-url=\"' + escapeHtml(item.url) + '\">';
                    html += '<div class=\"rent-search-item__title\">' + escapeHtml(item.title) + '</div>';
                    html += '<div class=\"rent-search-item__meta\">' + escapeHtml(item.area || '') + '</div>';
                    html += '</div>';
                });
            ...
            function fetchRentSuggestions(term){
                $.post(ajaxUrl, {
                    action: 'rent_search_suggestions',
                    nonce: $('#rentFilterForm input[name=\"nonce\"]').val(),
                    term: term
                }).done(function(res){
