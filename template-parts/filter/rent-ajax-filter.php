...
if (!defined('RENT_INDEX_META_OFFERING')) {
    define('RENT_INDEX_META_OFFERING', '_reaf_offering_type');
}
...
add_shortcode('porpertypi_ajax_filter_dynamic_rent', function () {
    ...
    $purpose_options = rent_get_distinct_meta_values(RENT_INDEX_META_OFFERING);
...
function rent_filter_porpertypi_dynamic() {
    ...
    // Always limit to Rent purpose so this filter only returns Rent properties
    $meta_query[] = [
        'key'     => RENT_INDEX_META_OFFERING,
        'value'   => array('For Rent', 'Rent'),
        'compare' => 'IN',
    ];

    if ($purpose !== '') {
        $low = strtolower(trim($purpose));
        if (!in_array($low, ['for rent', 'rent'], true)) {
            $meta_query[] = [
                'key'     => RENT_INDEX_META_OFFERING,
                'value'   => $purpose,
                'compare' => '='
    ...
    if ($q->have_posts()) {
        ...
        while ($q->have_posts()) {
            ...
            $pval = get_post_meta($id, RENT_INDEX_META_OFFERING, true);
...
function rent_search_suggestions() {
    ...
    // always only suggest from Rent properties
    $meta_query[] = [
        'key'     => RENT_INDEX_META_OFFERING,
        'value'   => array('Rent', 'For Rent'),
        'compare' => 'IN',
    ];
        ...
        <div class="rent-container">
            <div class="rent-cards__grid">
                <?php while ($q->have_posts()): $q->the_post();
                    ...
                    $purpose       = get_post_meta($id, RENT_INDEX_META_OFFERING, true);
