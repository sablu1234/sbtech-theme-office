...
function index_filter_porpertypi_dynamic() {
    ...
    $price_clause = index_build_price_meta_clause($min_price, $max_price);
...
$price_raw = get_post_meta($id, INDEX_META_PRICE, true);
...
function index_build_price_meta_clause($min_price, $max_price) {
    $clauses = [];
    $price_keys = [INDEX_META_PRICE];
        ...
        (function($){
            ...
            function renderSuggestions(items, term){
                ...
                items.forEach(function(item){
                    ...
                    const unitReference = String(item.unit_reference || '');
                    const searchText = title;

                    html += '<div class=\"index-search-item\" data-search=\"' + escapeHtml(searchText) + '\" data-url=\"' + escapeHtml(item.url) + '\">';
                    html += '<div class=\"index-search-item__title\">' + escapeHtml(title) + '</div>';

                    if(address){
                        html += '<div class=\"index-search-item__meta\">Address: ' + escapeHtml(address) + '</div>';
                    }

                    if(area){
                        html += '<div class=\"index-search-item__meta\">Area: ' + escapeHtml(area) + '</div>';
                    }

                    if(unitReference){
                        html += '<div class=\"index-search-item__meta\">Unit Ref: ' + escapeHtml(unitReference) + '</div>';
                    }
            ...
            function fetchSuggestions(term){
                ...
                $.post(ajaxUrl, data).done(function(res){
                    ...
                    renderSuggestions(res.data.items || [], term);
                }).fail(function(){
                    hideSuggestions();
                });
            }
            ...
            function fetchProps(page){
                ...
                $.post(ajaxUrl, data).done(function(res){
                    ...
                    let found = res.data.found || 0;
                    if (!res.data.html || !String(res.data.html).trim()) {
                        found = 0;
                    }
                    $('#reCount').text(found + ' results');
            ...
            $(document).on('click', '.index-search-item', function(){
                ...
                $('#reFilterForm input[name=\'paged\']').val(1);
                hideSuggestions();
                fetchProps(1);
            });
